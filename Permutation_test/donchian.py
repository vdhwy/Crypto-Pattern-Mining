...
if __name__ == '__main__':
    ...
    df = pd.read_csv('BTCUSDT_5m.csv', index_col='timestamp', parse_dates=True)

    df = df[(df.index.year >= 2023) & (df.index.year < 2025)] 
    best_lookback, best_real_pf = optimize_donchian(df)
    print(f"best_lookback: {best_lookback}, best_real_pf: {best_real_pf}")
    ...
    plt.savefig('donchian_test_results.jpeg', format='jpeg')
    plt.show()
